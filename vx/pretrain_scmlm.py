...
def _paragraph_length(dataset_path, _):
    all_batches = tf.data.experimental.load(dataset_path)
    paragraph_length = 0
    for paragraph in all_batches:
        paragraph_length = max(paragraph_length, paragraph.shape[0])
    return paragraph_length

def _generate_dataset_width(dataset_path, tokenizer):
    """ looks up the maximum tokens length possible in the dataset """
    all_batches = tf.data.experimental.load(dataset_path).unbatch()
    dataset_width = 0
...
def _setup_cached_args(tokenizer_filename, tokenizer_setup, dataset_path):
    ...
    return cache_values("configs/pretrain_scmlm_cache.yaml", {
        "paragraph_length": _paragraph_length,
        "dataset_width": _generate_dataset_width,
...
class PretrainerPipeline:
    ...
    def __init__(self, logger, outdir,
            dataset_config="configs/scmlm_dataset.yaml"):
    ...
    def setup_pretrainer(self, ckpt_id, ckpt_options=None, in_model_dir=None):
        ...
        preprocessor = model.PretrainerPreprocessor(self.preprocessor_dir,
                optimizer, builder.build(), self.tokenizer_setup, 'scmlm')
    ...
    def e2e(self,
        ...
        run_test = mlm.build_tester(preprocessor, pretrainer,
                ...
                mask_sizes=(self.training_args["batch_size"], self.cached_args["paragraph_length"]),
                logger=self.logger)

        # dataset
        training_batches = dataset.setup_scmlm_dataset(self.dataset_path, self.training_args,
                preprocessor.tokenizer, self.cached_args['paragraph_length'], logger=self.logger)
        if training_preprocessing is not None:
            training_batches = training_preprocessing(training_batches)

        # training
        train_batch, train_loss, train_accuracy = scmlm.build_pretrainer(
                preprocessor, pretrainer, optimizer,
                self.cached_args["dataset_width"],
                self.cached_args["paragraph_length"])
