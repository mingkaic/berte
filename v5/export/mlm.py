...
def build_tester(action_module, samples, batch_size, logger):
    """ build_tester returns a callable that tests the samples """
    n_extras = batch_size-len(samples)
    ...
    masked = mask_lm(tokens, lengths, 0.1, mask_id=action_module.metadata.mask())
...
def build_pretrainer(action_module, optimizer):
    ...
    def _call(batch, lengths, mask_rate):
        masked = mask_lm(batch, lengths, mask_rate, mask_id=action_module.metadata.mask())
