...
def _paragraph_length(dataset_path):
...
def _setup_cached_args(dataset_path):
...
class PretrainerPipeline:
    ...
    def __init__(self, logger, outdir):
        ...
        with open("configs/scmlm_dataset.yaml") as file:
            _args = yaml.safe_load(file.read())
        ...
        self.cached_args = _setup_cached_args(self.dataset_path)
    ...
    def e2e(self,
        ...
        training_batches = dataset.setup_scmlm_dataset(self.dataset_path, self.training_args,
                tokenizer, self.cached_args['paragraph_length'], logger=self.logger)
        if training_preprocessing is not None:
            training_batches = training_preprocessing(training_batches)

        for (batch, length) in training_batches.take(3):
            print(batch, length)

        # training
        # train_batch, train_loss, train_accuracy = mlm.build_pretrainer(
                # pretrainer, optimizer, self.cached_args["paragraph_length"])
